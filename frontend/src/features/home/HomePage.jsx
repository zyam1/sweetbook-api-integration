...
import {
  ...
  anthologyBadgeClass,
} from "../anthology/anthologyStatus";
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    listAnthologies("owner")
      ...
      .finally(() => {
        if (alive) setOwnedLoading(false);
      });
        ...
        return (
          ...
          >
            <div className="ant-row-between">
              ...
              <Badge
                variant={
                  a.latestOrder?.status
                    ? orderBadgeClass(a.latestOrder.status)
                    : anthologyBadgeClass(a.status || "DRAFT")
                }
              >
                {a.latestOrder?.status
      ...
      <section style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div className="home-grid-3">
          <Link to="/anthology" className="home-shortcut-card">
            ...
            <div className="home-shortcut-desc">
              내가 주최하거나 참여 중인 모든 합동지
            </div>
          </Link>
